...
impl Command {
    pub fn parse(input: &str) -> Result<(Vec<usize>, Command), &'static str> {
        let (input, destinators) = read_destinators(input.as_bytes())?;
        let input = unsafe { std::str::from_utf8_unchecked(input) };
        let input = input.trim();

        if input == "debug-state" || input == "debug_state" {
            return Ok((destinators, Command::DebugState));
        }

        Ok((destinators, Command::Unparsable(input.to_string())))
    }
}
